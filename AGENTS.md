...
- `SettingsController` — a **`GetxService`** (the name is kept for continuity), `permanent`, registered through `InitialBinding.initServices()` and **awaited before the first frame** ([#59](https://github.com/Teixeira49/bcv_tracker_app/issues/59)). Persists **three** settings via `SharedPreferences`: language, theme and the market the app opens on (`favMarketIndex`, a `TabController` index — *not* a set of followed markets; nothing in the app stores one yet, and #37's wording implying a fourth setting was taken from the plan, not from the code). Startup applies the stored values by **passing** them to `GetMaterialApp` (`themeMode:`, `locale:`), not by calling `Get.changeThemeMode` / `Get.updateLocale` — `GetMaterialApp.initState` assigns `Get.locale` from its own argument, so a locale applied before `runApp` was overwritten on the spot. The setters still use the framework calls; those apply a change to a running app, which is a different job
...
- **The settings screen has no controller of its own** ([#37](https://github.com/Teixeira49/bcv_tracker_app/issues/37)). `SettingsPage` and its three sub-screens read `SettingsController` with `Obx` and call its setters; a controller there would be a second object holding the same three values. `GetBuilder` is not an option and would be a trap: the service never calls `update()`, so it would locate the instance and then never rebuild
...
10 languages via GetX `Translations`. Language maps in `lib/core/i18n/languages/` (85 keys, all ten in parity, and `AppMessages` exposes exactly those 85). Use `AppMessages.<key>` for all UI text — never a raw string, and never `'<key>'.tr` in a widget: `app_messages.dart` is the only file that calls `.tr`. A new key goes into all ten files in the same commit, or GetX renders the raw key on screen for the other nine.
...
**Six routes: the splash, the dashboard, and settings with its three choice sub-screens** (`/settings`, `/settings/market`, `/settings/language`, `/settings/theme`). Settings was a dialog until [#37](https://github.com/Teixeira49/bcv_tracker_app/issues/37); what moved it was not the look but the ceiling — a dialog does not grow, and the settings queued behind it (#13, #33, #34, about) do not fit in one at any font scale. The three sub-routes nest their names under `/settings` deliberately: it says the screen is reached from the menu, which is what a deep link into `/settings/language` should mean. Adding a setting is a `SettingsMenuTile` in `settings_menu.dart` plus, if it needs a list, a `GetPage` rendering `SettingsOptionsPage<T>` — nothing on the menu gets rearranged.
...
**Golden tests** guard the UI against visual regressions ([#35](https://github.com/Teixeira49/bcv_tracker_app/issues/35)): `AppStateView` (error/empty), the variation indicator, the Home cards, the converter body, the currency detail and selector sheets, and the settings menu with its language sub-screen ([#37](https://github.com/Teixeira49/bcv_tracker_app/issues/37)) — each in light and dark. They use **Alchemist** (`*_golden_test.dart`), configured in `test/flutter_test_config.dart` to emit **only CI goldens** — text is obscured as blocks so the references render identically on the Linux CI and on a local Mac (icons, SVGs, palette and layout still render for real). Regenerate with `flutter test --update-goldens` **only** on an intended UI change, and review the `*.png` diff. Full procedure in [`docs/golden-tests.md`](docs/golden-tests.md).
