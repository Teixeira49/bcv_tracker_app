...
> **Adaptada a este repositorio** (origen: `dhruvanbhalara/skills`, MIT). Los nombres genéricos de tokens y carpetas del original se traducen así:
>
> | En el original | Aquí |
> |---|---|
> | `AppColors` | `AppColors` — `lib/config/theme/colors/colors_constants.dart` ✅ mismo nombre |
> | `AppSpacing.sm` / `.md` | `WidthValues.spacingSm` / `.spacingMd` — `lib/config/theme/width/width_values.dart` |
> | `AppRadius.md` | `WidthValues.radiusMd` |
> | `AppTypography` | no existe todavía; usa `context.textTheme` |
> | `core/views/widgets` | `lib/shared/presentation/widgets/` (o el `widget/` del feature si no se reutiliza) |
>
> **Estado real:** `WidthValues` está declarado pero **ningún widget lo usa** — los 21 `EdgeInsets` de la app llevan números sueltos. La sección 2 de abajo es, literalmente, la deuda pendiente; ver [`.agents/rules/constants-centralization.md`](../../rules/constants-centralization.md).
>
> **Material 3:** el `context.colorScheme` que recomienda esta skill asume M3. La app aún es Material 2 (`ThemeData` sin `useMaterial3`, paletas `MaterialColor`); la migración está decidida pero no hecha, así que `AppColors` sigue siendo la vía hasta que ocurra. Ver `flutter-expert/references/theming.md` para el camino de migración.
