...
- `AppInfoService` — a **`GetxService`**, `permanent`, registered through `InitialBinding.initServices()` and awaited before the first frame. Holds the installed package's `version` and `buildNumber`, read at runtime with `package_info_plus` ([#43](https://github.com/Teixeira49/bcv_tracker_app/issues/43)) — never from a constant, which desynchronises on the first release someone forgets to bump. The first frame does **not** read it; it is awaited anyway because `dependencies()` is not, and a `Get.find` reaching an unresolved `putAsync` throws. One service, so the settings menu and «Acerca de» cannot quote different builds. The build number **will not match** the `+N` in `pubspec.yaml` on a release: `codemagic.yaml` passes `CM_BUILD_NUMBER`, and what a bug report needs is the artefact the tester actually has
