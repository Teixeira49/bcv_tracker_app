...
10 languages via GetX `Translations`. Language maps in `lib/core/i18n/languages/` (113 keys, all ten in parity, and `AppMessages` exposes exactly those 113). Use `AppMessages.<key>` for all UI text — never a raw string, and never `'<key>'.tr` in a widget: `app_messages.dart` is the only file that calls `.tr`. A new key goes into all ten files in the same commit, or GetX renders the raw key on screen for the other nine.
