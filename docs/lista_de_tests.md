...
## Cobertura actual (53 archivos, 378 tests)
...
| | `test/features/settings/settings_about_page_test.dart` | **(#42)** «Acerca de»: las nueve fuentes con su tipo de referencia, que **cada fila abre su propio origen y no otro**, que los nombres de mercado no se traducen, licencia y ambos repositorios, que el enlace de la documentación sigue al backend configurado (y no a uno fijo), el formulario de issue —no la lista—, y que todas las filas llevan el icono de «sale de la app» y ninguna el chevron |
| | `test/core/helpers/external_link_test.dart` | **(#42)** La única puerta de salida: abre `https`, **rechaza `http`, `tel:`, `intent:`, `javascript:`, `file:` y basura sin llegar a la plataforma**, y degrada a `false` cuando la plataforma declina o lanza |
