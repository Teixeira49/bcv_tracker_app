...
## Cobertura actual (54 archivos, 390 tests)
...
| | `test/features/settings/app_version_test.dart` | **(#43)** La versión: que la etiqueta compone versión y build, que el menú muestra **la del paquete instalado** y no una constante (el fake dice 9.9.9 y el `pubspec` otra cosa), que la fila **copia y no navega** y su icono lo dice, que tocarla deja la cadena en el portapapeles y lo confirma, y que **«Acerca de» muestra exactamente el mismo dato** — el criterio duro del issue |
