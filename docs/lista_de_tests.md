...
## Cobertura actual (52 archivos, 385 tests)
...
| | `test/features/settings/settings_page_golden_test.dart` | **(#37)** El menú de ajustes, la subpantalla de idioma, la **rejilla del tema** y el **contador de decimales**, claro y oscuro — esta última es la que cazó que las tarjetas se pintaban debajo del panel por usar `Ink` en vez de un `Material` propio |
